...
from .detection_utils import (annotations_to_instances, asso_annotations_to_instances, build_augmentation,
...
def calculate_padding_mask(image, tfm_gens):
    """
    Calculate the padding mask for an image.

    Args:
        image (np.array): The original image.
        tfm_gens (list[TransformGen]): A list of :class:`TransformGen` to be applied.

    Returns:
        padding_mask (torch.Tensor): The padding mask.
    """

    # Initialize a padding mask with the same size as the image
    padding_mask = np.ones(image.shape[:2])
    # Apply the transforms to the image
    image, transforms = T.apply_transform_gens(tfm_gens, image)
    # Apply the same transforms to the padding mask
    padding_mask = transforms.apply_segmentation(padding_mask)
    # Get the padding areas
...
class DatasetMapperWithBasis(DatasetMapper):
    ...
    def __call__(self, dataset_dict):
        ...
        # dataset_dict["padding_mask"] = calculate_padding_mask(image, self.augmentation)
        ...
        if "annotations" in dataset_dict and "association_anno" in dataset_dict:
            ...
            for asso_anno in dataset_dict["association_anno"]:
                if not self.use_instance_mask:
                    asso_anno.pop("segmentation", None)
                if not self.use_keypoint:
                    asso_anno.pop("keypoints", None)
            ...
            if self.is_train:
                if np.random.rand() > 0.5 and len(annos)<10:
                    annos, asso_annos, image = self.double_object(annos, asso_annos, image)
                    dataset_dict["image"] = torch.as_tensor(
                        np.ascontiguousarray(image.transpose(2, 0, 1))
                    )
            ...
            ##### association annos #####
            asso_instances = asso_annotations_to_instances(
                asso_annos, image_shape, mask_format='bitmask'
            )
            if self.recompute_boxes:
                asso_instances.gt_boxes = asso_instances.gt_masks.get_bounding_boxes()
            dataset_dict["association_anno"] = asso_instances
        ...
        if hasattr(instances, 'gt_masks'):
            dataset_dict["instances"].gt_masks = dataset_dict["instances"].gt_masks.tensor.to(torch.int32)
    ...
    def double_object(self, annos, asso_annos, image):
        ...
        def object_shift(mask,w,h,shadow,img,fullmask):
            ...
            image = res.astype('uint8')
            obj_mask = obj_mask>0
            sha_mask = shadow_mask[:,:,0]>0
            asso_mask = obj_mask | sha_mask
            # return res.astype('uint8') , obj_mask>0, shadow_mask[:,:,0]>0
            return image, obj_mask, sha_mask, asso_mask
        ...
        for random_obj1 in random_nums:
            ...
            # For each randomly selected target instance (random_obj1), first calculate its new position (shadow) in the image,
            # then call the object_shift function to copy and paste the target instance to the new position. After copying and pasting,
            # you will get a new image (image), the new position of the target instance (obj_mask), and the new position of the shadow (sha_mask).

            shadow = random_obj1 + obj_num # Shadow's new position subscript
            ...
            image, obj_mask, sha_mask, asso_mask = object_shift(masks[random_obj1],w1,h1, masks[shadow], image, full_mask)



            sha_mask_small = sha_mask[1::2,1::2]
            obj_mask_small = obj_mask[1::2,1::2]
            asso_mask_small = asso_mask[1::2,1::2]
            if sha_mask_small.sum() > 300 and obj_mask_small.sum() > 300 and asso_mask_small.sum() > 300:

                # First, check if the number of pixels in the new shadow area and the target instance area is greater than 300. If so, continue processing.
                # If the number of pixels in the new shadow area and the target instance area is not greater than 300, only update the segmentation information 
                # and bounding boxes of other target instances, as well as the relationship information of all target instances.

                # First, create a new target instance (new_obj) and shadow (new_obj_sha), and add them to the annotations of the target instances.
                new_obj = {
                    'iscrowd': 0,
                    'category_id': 0,
                ...
                new_asso = {
                    'iscrowd': 0,
                    'category_id': 0,
                    'segmentation': asso_mask,
                    'bbox': BoxMode.convert(maskUtils.toBbox(maskUtils.encode(np.array(asso_mask[:,:,None], order='F',dtype='uint8'))[0])[None], BoxMode.XYWH_ABS, BoxMode.XYXY_ABS)[0],
                    'bbox_mode': annos[0]['bbox_mode'],
                    'light': maskUtils.toBbox(maskUtils.encode(np.array(asso_mask[:,:,None], order='F',dtype='uint8'))[0])
                }

                # Then, update the segmentation information and bounding boxes of other target instances, as well as the relationship information of all target instances.
                # Finally, update the number of target instances (obj_num) and the list of target instances (masks).
                ...
                # If the number of pixels in the new shadow area and the target instance area is not greater than 300,
                # then only update the segmentation information and bounding boxes of other target instances, as well as the relationship information of all target instances.
