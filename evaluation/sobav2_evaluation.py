...
palette = sns.color_palette('hls', n_colors=20)  # N is the maximum number of instances
...
class SOBAEvaluator(DatasetEvaluator):
    ...
    def __init__(self, dataset_name, cfg, distributed, output_dir=None):
        ...
        self._annotations_datasetroot = cfg['INPUT']['ANNOTATIONS_DATASETROOT']
    ...
    def _eval_predictions(self, tasks):
        ...
        print("*"*88)
        print("*"*88)
        print(" ------------------ start of evaluating SOAP_assoAP ------------------ ")

        self.evaluate_SOAP_assoAP(val_file_path=self._annotations_datasetroot + "/SOBA_val_v2.json", json_file_path=self._output_dir)

        print(" ------------------ end of evaluating SOAP_assoAP ------------------ ")
        print("*"*88)
        print("*"*88)
        print("*"*88)
    ...
    def evaluate_SOAP_assoAP(self, val_file_path, json_file_path):
        soba = SOBA(val_file_path)
        results =  json.load(open('{}/soba_instances_results.json'.format(json_file_path)))
        association = json.load(open('{}/soba_association_results.json'.format(json_file_path)))
...
def instances_to_json(instances, img_id):
    ...
    has_locations = instances.has("locations")
    has_fpn_levels = instances.has("fpn_levels")
    has_offset = instances.has("offset")
    if num_instance == 0:
        return []
    try:
        ...
        if has_locations:
            locations = instances.locations.numpy().tolist()
        if has_fpn_levels and has_offset:
            fpn_levels = instances.fpn_levels.numpy().tolist()
            offset = instances.offset.numpy().tolist()
    except:
    ...
    for k in range(num_instance):
        ...
        if has_locations:
            result["locations"] = locations[k]
        if has_fpn_levels and has_offset:
            result["fpn_levels"] = fpn_levels[k]
            result["offset"] = offset[k]
        if has_keypoints:
            # In SOBA annotations,
